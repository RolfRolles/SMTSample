...
class TestX86TypeChecker(VerboseTestCase):
	tc = X86TypeChecker()
	def do_test(self,aop,possibility_list):
		for (possibility,result) in possibility_list:
			if self.verbose:
				print "Testing", aop, "against possibility", possibility, "expecting result", result
			...
			if result is not None:
				...
				bFailed = res is not None
			if res is not None:
				error = "X86TypeChecker(%s,%s) should have returned a TypeCheckInfo object, not type %s" % (aop,possibility,type(res))
				...
				bFailed |= not(res.sizeo == result.sizeo)
				...
				bFailed |= not(res.addro == result.addro)
				...
				bFailed |= not(res.sego == result.sego)
			if self.verbose and not bFailed:
				print "%s matching %s with result %s: test passed!" % (aop,possibility,result != None)
