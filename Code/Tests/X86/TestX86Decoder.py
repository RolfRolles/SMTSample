...
class TestX86Decoder(VerboseTestCase):
	...
	def do_test(self,instr,bytes):
		if self.verbose:
			print "Testing bytes", bytes, "expecting instruction", instr
		...
		bFailed = not(isinstance(i2container,X86DecodedInstruction))
		i2 = i2container.instr
		self.assertIsInstance(i2,Instruction,self.err2+repr(i2))
		bFailed |= not(isinstance(i2,Instruction))
		error = "%s: expected %s(%r), got %s(%r)" % (bytes,instr,instr,i2,i2)
		self.assertEqual(instr,i2,error)
		bFailed |= instr != i2
		if self.verbose and bFailed:
			print "%s: matched decoded bytes (test passed)!" % instr
