...
def FixSegment(mem,sego):
	return mem if sego else mem(mem.DefaultSeg())
...
class X86RandomOperand(Visitor2):
	...
	def gen(self,aop,(m,sego,s,a)):
		return self.visit(AOTtoAOTDL[aop.IntValue()],(m,sego,s,a))
	...
	def visit_AddrPrefix(self,addr,(m,sego,s,a)):
		return self.visit_Prefixed(addr.yes,addr.no,a,(m,sego,s,a))

	# Call visit_Prefixed using the size-override boolean s
	def visit_SizePrefix(self,size,(m,sego,s,a)):
		return self.visit_Prefixed(size.yes,size.no,s,(m,sego,s,a))

	# To "randomly generate" implicit operands, return them directly.
	def visit_Exact(self,i,(m,sego,s,a)):
		return i.value
	
	# For implicit operands that may have their segments overridden, make a copy
	# of the operand and put a random segment into it.
	def visit_ExactSeg(self,i,(m,sego,s,a)):
		return FixSegment(i.value(rnd_seg()),sego)

	# Generate a random register of the same type as the archetype.
	def visit_GPart(self,g,(m,sego,s,a)):
	...
	# 8 elements in them).
	def visit_GPart_SegReg(self,g,(m,sego,s,a)):
	...
	def visit_RegOrMem(self,mrm,(m,sego,s,a)):
		...
		if a:
			...
			return FixSegment(Mem16(rnd_seg(),size,b,i,rnd_displ(b is not None,0xFFFF)),sego)
		...
		# If a is False, generate a Mem32 object
			...
			return FixSegment(Mem32(rnd_seg(),size,b,sr,sf,d),sego)
	...
	def visit_Immediate_MemExpr(self,i,(m,sego,s,a)):
		rndseg = rnd_seg()
		if a: return FixSegment(Mem16(rndseg,i.archetype.size,None,None,rnd_word()),sego)
		else: return FixSegment(Mem32(rndseg,i.archetype.size,None,None,0,rnd_dword()),sego)
	...
	def visit_Immediate_FarTarget(self,i,(m,sego,s,a)):
	...
	def visit_Immediate_JccTarget(self,i,(m,sego,s,a)):
		d = rnd_dword()
		return JccTarget(d & 0xFFFF if a else d,rnd_dword())

	# For Immediate constants, generate random constants.
	def visit_Immediate_Id(self,i,(m,sego,s,a)): return Id(rnd_dword())
	def visit_Immediate_Iw(self,i,(m,sego,s,a)): return Iw(rnd_word())
	def visit_Immediate_Ib(self,i,(m,sego,s,a)): return Ib(rnd_byte())
	...
	def visit_SignExtImm_Iw(self,i,(m,sego,s,a)):
		return Iw(self.visit_SignExtImm_Common(0xFFFF))

	# Return an Id of a suitable constant.
	def visit_SignExtImm_Id(self,i,(m,sego,s,a)):
...
def generate_random_instruction(excl=(lambda x: False)):	
	...
	mem,sego,so,ao = rnd_bool(),rnd_bool(),rnd_bool(),rnd_bool()
	rog = X86RandomOperand()
	reified_ops = map(lambda o: rog.gen(o,(mem,sego,so,ao)),enc.ops)
